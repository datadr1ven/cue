...
Same shared module as TPlus (`packages/cue/src/telegram-inbox.js`):
...
- `apps/gridwhisper/worker/**`
- `apps/gridwhisper/wrangler.toml`
- `apps/gridwhisper/src/gridwhisper-commands.js`
- `packages/cue/src/telegram-inbox.js`
...
**Not** redeployed by this workflow: F1 engine / moments (`packages/cue/src/engine/**`). Those run on your laptop via MQTT → `POST /deliver`. Pull + restart the local worker for those.

Manual deploy anytime: **Actions → Deploy GridWhisper → Run workflow**, or:

```bash
npx wrangler deploy -c apps/gridwhisper/wrangler.toml
```
...
# one-time KV
npx wrangler kv namespace create GRIDWHISPER_KV -c apps/gridwhisper/wrangler.toml
```

Paste the namespace **id** into `apps/gridwhisper/wrangler.toml`:
...
```bash
npx wrangler secret put TELEGRAM_TOKEN -c apps/gridwhisper/wrangler.toml
# paste GridWhisper bot token

npx wrangler secret put DELIVER_SECRET -c apps/gridwhisper/wrangler.toml
# paste a long random string (keep for race-day .env)

# optional (future ops)
# npx wrangler secret put TELEGRAM_ADMIN_IDS -c apps/gridwhisper/wrangler.toml
...
In `apps/gridwhisper/wrangler.toml` `[vars]`:
